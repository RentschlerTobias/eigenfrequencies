#!/bin/sh
# MPI launcher: bwUniCluster provides mpiexec, the dtOO container only mpirun
# (openmpi4). Override with MPI_LAUNCHER; eigenfrequencies passes it from
# case.options.cfd.mpi_launcher. Default unchanged, so the cluster path is the
# de_framework original.
...
for i in "_n_";do
    ...
    "${MPI_LAUNCHER:-mpiexec}" --oversubscribe -n $procPerJob -v simpleFoam -parallel -fileHandler collated > log.mpiexec 2>&1
