...
# objective = cfd_scalar(eta, vcav, dH). Defaults above are the smoke test
# (6 nodes x 4 workers x 16 cores for simpleFoam, POP_SIZE=24 derived from
# nodes x tasks, MAX_GEN=1).
#
# Production override at submission time, e.g.:
#   sbatch --partition=cpu_il --nodes=4 --ntasks-per-node=4 --cpus-per-task=16 \
#          --time=08:00:00 --export=ALL,DE_POP_SIZE=16,DE_MAX_GEN=20 \
