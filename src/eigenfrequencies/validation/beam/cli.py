"""Interactive TUI and validation helpers for the beam modal analysis demo."""
...
from eigenfrequencies.validation.beam.analytical import analytical_frequencies

# Ensure demo/beam modules are importable when running from the package.
_BEAM_DIR = os.path.join(
...
class BeamTUI(App):
    ...
    def run_simulation(self) -> None:
        ...
        self.write_log(
            f"BCs: left={'clamped' if tui_cfg.clamped_left else 'free'}, "
            f"right={'clamped' if tui_cfg.clamped_right else 'free'}"
        )
