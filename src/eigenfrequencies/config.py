...
Moved from ``turbine_runner/config.py`` with two critical fixes:

1. ``n_rpm`` is now a **required** field (no import-time XML read).
   The dtOO adapter supplies it at runtime.
2. ``CFDConfig.omega`` is computed from ``n_rpm`` in ``__post_init__``,
...
class OptimizationConfig:
    ...
    Attributes:
        n_rpm: Runner rotational speed in rpm (supplied at runtime by the dtOO adapter)
...
class CFDConfig:
    ...
    Attributes:
        n_rpm: Runner rotational speed in rpm (required; drives omega)
        omega: Runner angular velocity in rad/s (P = moment * omega);
            computed as 2*pi*n_rpm/60 in __post_init__ if not provided
    ...
    def __post_init__(self):
        if self.omega is None:
            self.omega = 2.0 * math.pi * self.n_rpm / 60.0
