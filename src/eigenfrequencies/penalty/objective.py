...
Dependency-light (numpy only, plus penalty.compute_penalty) so it imports on
...
def cfd_scalar(cfd: dict, cfd_cfg: CFDConfig, obj_cfg: ObjectiveConfig) -> float:
...
def resonance_term(frequencies, opt_cfg: OptimizationConfig, obj_cfg: ObjectiveConfig) -> float:
...
def combined_objective(cfd, frequencies, cfd_cfg: CFDConfig, opt_cfg: OptimizationConfig, obj_cfg: ObjectiveConfig) -> tuple:
