"""Mesh loading and volume verification.

Ported from ``turbine_runner/mesh_prep.py`` with two changes:

1. Uses ``eigenfrequencies.config.MeshConfig`` instead of the legacy
   ``turbine_runner.config.MeshConfig``.
2. ``MeshVerificationError`` replaces the generic ``RuntimeError`` so callers
   can distinguish mesh problems from other failures.
"""
...
class MeshVerificationError(RuntimeError):
    """Raised when a mesh fails volume verification (no 3-D cells or zero volume)."""
    ...
    try:
        ...
        if not surfaces:
            raise MeshVerificationError(
                f"No surfaces found in {surf_msh}; cannot build a volume."
            )
...
def load_and_prepare_mesh(mesh_cfg: MeshConfig):
    ...
    if domain.topology.dim != 3:
        raise MeshVerificationError(
            f"Fallback failed to produce a 3-D volume mesh: {out_msh}"
        )
    return domain
